...
const X402_API_URL = process.env.X402_API_URL || "https://api.truecast.example.com/enrich-event";
const ART_STYLES = [
  "Studio Ghibli style in modern setting. Whimsical.", 
...
/**
 * Initialize CDP wallet client for x402 payments
 */
async function initializeCdpClient() {
...
async function getNewsworthyEvents(maxPosts: number = MAX_NEWS_POSTS): Promise<ProcessedNewsworthyEvent[]> {
  ...
  try {
    ...
    const rawEventStrings = await redis.lrange(newsworthyEventsKey, 0, MAX_NEWS_POSTS * 3 - 1);
    ...
    // --- x402 API Enrichment ---
    const enrichedEvents = await enrichEventsWithX402(eventsToProcess);

    if (enrichedEvents.length === 0) {
        console.log("No events were processed with x402 API. Cannot rank.");
        return [];
    }
    ...
    const rankSchema = z.object({
      rankedEvents: z.array(z.object({
        ...
        imagePrompt: z.string().describe("Image prompt for AI-generated art for this event. Should be allegory for the event capturing its essence as closely as possible. Should be fun, joyous and without text or famous likenesses.")
      }))
    });

    const rankPrompt = `You are a news analyst/journalist specializing in prediction markets.

Below are ${enrichedEvents.length} events from prediction markets with additional context from TrueCast API. Please:
...
3. Write a brief, compelling paragraph (max 255 characters) to display on a news website for each, describing the event and why it's significant. Do not talk about the prediction market itself, write it like a news article. No em dashes, no hyphens.
...
${enrichedEvents.map((event, idx) => {
    ...
    if (event.x402Analysis) {
      details += `\n\nTrueCast API Analysis: ${event.x402Analysis}`;
    } else {
...
/**
 * Enriches events with analysis from TrueCast x402 API
 * @param eventsToProcess Array of events to enrich
 * @returns Promise with array of enriched events
 */
async function enrichEventsWithX402(
  eventsToProcess: ProcessedNewsworthyEvent[]
): Promise<ProcessedNewsworthyEvent[]> {
  console.log(`Enriching ${eventsToProcess.length} events with x402 API...`);
  
  // Initialize CDP wallet client for x402 payments
  const account = await initializeCdpClient();
  ...
  for (let i = 0; i < eventsToProcess.length; i++) {
    ...
    const enrichedEvent: ProcessedNewsworthyEvent = { ...event };

    try {
      console.log(`Making x402 API request for event: ${event.marketQuestion}`);
      ...
      if (response.ok) {
        ...
        enrichedEvent.x402Analysis = data.analysis || data.context || data.result;
        
        // Log payment information if available
        const paymentResponse = response.headers.get("x-payment-response");
        if (paymentResponse) {
          const decodedPayment = decodeXPaymentResponse(paymentResponse);
          console.log(`Payment completed for event ${event.marketId}:`, decodedPayment);
        }
        
        console.log(`Successfully enriched event ${event.marketId} via x402 API`);
      } else {
        console.warn(`x402 API request failed for event ${event.marketId}: ${response.status} ${response.statusText}`);
      }
    } catch (error) {
      console.error(`Error calling x402 API for event ${event.marketId}:`, error);
    } finally {
...
async function postEvent(
  ...
  try {
    ...
    if (DISABLE_POSTS) {
      ...
      if (event.x402Analysis) {
        console.log(`- x402 Analysis: "${event.x402Analysis}"`);
      }
      return;
    }
...
async function postNews(maxPosts: number = MAX_NEWS_POSTS) {
  console.log(`Starting to post newsworthy events (max: ${maxPosts}) using x402 API...`);
